    ...
    try:
        ...
        for key, expected_type in required_keys.items():
            ...
            if not isinstance(config_data[key], expected_type):
                logging.error(
                    f"错误：配置文件 '{config_file_path}' 中键 '{key}' 的类型不正确。期望类型：{expected_type}, 实际类型：{type(config_data[key])}。")
                return None
...
def get_text_content_for_leaf(leaf_node_data: Dict, all_text_files: List[Path], text_dir_base_path: Path,
                              chapter_path_id_for_log: str) -> Optional[str]:
    """读取并连接给定叶节点的文本内容。"""
    ...
    if not start_file_name or not end_file_name or \
        ...
        logging.warning(
            f"节点 '{leaf_node_data.get('name')}' (生成ID: {chapter_path_id_for_log}) 缺少有效的文件名范围 ({start_file_name} - {end_file_name})。跳过。")
        return None
    ...
    try:
        ...
        if start_file_name not in filename_to_path or end_file_name not in filename_to_path:
            logging.warning(
                f"节点 '{leaf_node_data.get('name')}' (生成ID: {chapter_path_id_for_log}) 的起始/结束文件 ({start_file_name} / {end_file_name}) 在文件列表中未找到。")
            return None
        ...
        if start_idx > end_idx:
            logging.warning(
                f"节点 '{leaf_node_data.get('name')}' (生成ID: {chapter_path_id_for_log}) 的起始文件索引 ({start_idx}) 大于结束文件索引 ({end_idx})。将只使用起始文件。")
            end_idx = start_idx

        content_parts = []
        logging.info(
            f"为节点 {chapter_path_id_for_log} 读取文件范围 {start_file_name} 到 {end_file_name} (索引 {start_idx} 到 {end_idx})")
        ...
        logging.error(
            f"文件名 '{start_file_name}' 或 '{end_file_name}' 未在排序的文件列表中找到 (节点生成ID: {chapter_path_id_for_log})。")
        return None
    except Exception as e:
...
def call_llm_for_orgchart_nodes(api_key: Optional[str], chapter_path_id: str, chapter_name: str, chapter_content: str,
    ...
    system_prompt = PROMPT_ORGCHART_CHAPTER_TEMPLATE.replace("{path_id_placeholder}",
                                                             chapter_path_id)  # path_id 现在是动态生成的
            ...
            if first_bracket != -1 and last_bracket > first_bracket:
                ...
                try:
                    ...
                    if isinstance(parsed_list, list):
                        logging.info(
                            f"章节 '{chapter_name}' 的OrgChart JSON列表解析成功，包含 {len(parsed_list)} 个节点。")
                        return parsed_list
                    else:
                    ...
                    try:
                        ...
                        if isinstance(chapter_internal_nodes, list):
                            for internal_node in chapter_internal_nodes:
                                ...
                                internal_node_pid_str = str(internal_node.get('pid')) if internal_node.get(
                            ...
                            logging.warning(
                                f"章节 {current_node_orgchart_id} 的OrgChart文件 '{chapter_orgchart_filename.name}' 内容不是列表。")
                    except Exception as e:
                        logging.error(
                            f"读取或处理章节 {current_node_orgchart_id} 的OrgChart文件 '{chapter_orgchart_filename.name}' 时出错: {e}")
                else:
                    logging.warning(
                        f"未找到章节 {current_node_orgchart_id} 的OrgChart文件: {chapter_orgchart_filename.name}")
...
def main():
    ...
    if not input_catalog_file.exists():
        logging.error(
            f"错误: 输入的目录文件 '{input_catalog_file}' 未找到。请先运行 get_catalog.py (确保它生成了层级结构和index)。")
        return
    try:
